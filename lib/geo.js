...
export function getTravelModes(restaurant, userLat, userLng) {
  ...
  // Add delivery mode if restaurant supports it
  const deliveryFee = restaurant.deliveryFee ?? restaurant.delivery_fee;
  ...
  if (deliveryFee !== undefined && deliveryFee !== null) {
    modes.push({
      ...
      icon: '🛵',
      label: 'Delivery',
      time: deliveryTime ? parseInt(deliveryTime) : estimateTravelTime(distance, 'drive') + 10,
      cost: deliveryFee,
      timeLabel: deliveryTime || `${estimateTravelTime(distance, 'drive') + 10} min`,
    });
  }
